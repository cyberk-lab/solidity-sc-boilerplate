...
StableToken `initialize()` SHALL accept `rewardRecipient_` and `dailyRewardCapBps_` parameters to configure reward minting state at deployment time.

#### Scenario: Initialize with reward config

- **WHEN** `initialize(admin, rewardRecipient, 100)` is called
- **THEN** `rewardRecipient` and `dailyRewardCapBps` are set correctly
